...
- **Trajectory boundary conditions** — `q(0) = q0`, `dq(0) = 0` for all basis/phase combinations; sine integer-period endpoint guarantee; config rejection of non-integer sine periods; EL+constrained identification rejected; `cholesky` accepted as distinct feasibility method
